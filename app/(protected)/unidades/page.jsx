...
import { getUnidades, createUnidad, updateUnidad, deleteUnidad } from '@/supabase/helpers/unidades'
import { getUsuariosByRol } from '@/supabase/helpers/usuarios'
...
export default function UnidadesPage() {
  ...
  const cargarPropietarios = useCallback(async () => {
    const { data } = await getUsuariosByRol(['propietario'])
  ...
  const cargarUnidades = useCallback(async () => {
    setLoading(true)
    try {
      const { data, count, error } = await getUnidades({
      ...
      notifications.show({ color: 'red', title: 'Error al cargar unidades', message: err.message ?? 'Intenta de nuevo.' })
    } finally {
      setLoading(false)
    }
  ...
  async function handleGuardar() {
    ...
    try {
      ...
      notifications.show({ color: 'red', title: modoEdicion ? 'Error al actualizar' : 'Error al crear', message: err.message ?? 'Ocurrió un error inesperado.' })
    } finally {
      setGuardando(false)
    }
  }
  ...
  async function handleEliminar() {
    ...
    setEliminando(true)
    try {
      const { error } = await deleteUnidad(unidadSeleccionada.id)
      if (error) throw error
      notifications.show({ color: 'green', title: 'Unidad eliminada', message: `Unidad ${unidadSeleccionada.numero} eliminada.` })
      cerrarEliminar()
      if (unidades.length === 1 && pagina > 1) setPagina((p) => p - 1)
      else cargarUnidades()
    } catch (err) {
      notifications.show({ color: 'red', title: 'Error al eliminar', message: err.message ?? 'No se pudo eliminar la unidad.' })
    } finally {
      setEliminando(false)
    }
  }
  ...
  return (
    <>
          ...
          {loading ? (
            ...
            <Center h={200}>
              <Stack align="center" gap="xs">
                ...
                {hayFiltros && (
                  <Button variant="subtle" size="xs" onClick={limpiarFiltros}>Limpiar filtros</Button>
                )}
              </Stack>
            </Center>
          ) : (
            <ScrollArea>
              <Table striped highlightOnHover verticalSpacing="sm" miw={760}>
                ...
                <Table.Tbody>
                  {unidades.map((u) => (
                    <Table.Tr key={u.id}>
                      <Table.Td><Text size="sm" fw={600}>{u.numero}</Text></Table.Td>
                      <Table.Td><Text size="sm">{u.torre ?? '—'}</Text></Table.Td>
                      <Table.Td><Text size="sm">{u.piso ?? '—'}</Text></Table.Td>
                      <Table.Td><Text size="sm" style={{ textTransform: 'capitalize' }}>{u.tipo ?? '—'}</Text></Table.Td>
                      <Table.Td>
                        <Badge size="sm" color={estadoColor[u.estado] ?? 'gray'} variant="light" style={{ textTransform: 'capitalize' }}>
                          {u.estado ?? '—'}
                        </Badge>
                      </Table.Td>
                      <Table.Td>
                      ...
                      <Table.Td><Text size="sm">{u.area_m2 != null ? `${u.area_m2} m²` : '—'}</Text></Table.Td>
                      <Table.Td>
                        <Group justify="center" gap="xs">
                          <Tooltip label="Editar">
                            <ActionIcon variant="light" color="blue" size="sm" onClick={() => abrirEditar(u)}>
                              <Pencil size={14} />
                            </ActionIcon>
                          </Tooltip>
                          <Tooltip label="Eliminar">
                            <ActionIcon variant="light" color="red" size="sm" onClick={() => confirmarEliminar(u)}>
                              <Trash2 size={14} />
                            </ActionIcon>
                          </Tooltip>
                        </Group>
          ...
          {!loading && totalPaginas > 1 && (
            <>
              ...
              <Group justify="space-between" p="md" wrap="wrap" gap="xs">
                ...
                <Pagination value={pagina} onChange={setPagina} total={totalPaginas} size="sm" withEdges />
              </Group>
            </>
          )}
        </Paper>
      ...
      <Modal
        opened={modalAbierto}
        onClose={cerrarModal}
        title={<Group gap="xs"><Home size={18} /><Text fw={600}>{modoEdicion ? 'Editar unidad' : 'Nueva unidad'}</Text></Group>}
        size="lg"
        centered
        overlayProps={{ blur: 3 }}
      >
        <Stack gap="sm">
          <Grid gutter="sm">
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <TextInput label="Número de unidad" placeholder="Ej: 101, A-05" required value={form.numero} onChange={(e) => setField('numero', e.target.value)} />
            </Grid.Col>
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <TextInput label="Torre / Bloque" placeholder="Ej: A, B, Norte" value={form.torre} onChange={(e) => setField('torre', e.target.value)} />
            </Grid.Col>
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <NumberInput label="Piso" placeholder="Ej: 3" min={-5} max={100} value={form.piso} onChange={(v) => setField('piso', v)} />
            </Grid.Col>
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <Select
                label="Tipo"
            ...
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <NumberInput label="Área (m²)" placeholder="Ej: 75.5" min={0} decimalScale={2} value={form.area_m2} onChange={(v) => setField('area_m2', v)} />
            </Grid.Col>
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <NumberInput label="Coeficiente (%)" placeholder="Ej: 1.25" min={0} max={100} decimalScale={4} value={form.coeficiente} onChange={(v) => setField('coeficiente', v)} />
            </Grid.Col>
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <Select
                label="Propietario"
          ...
          <Group justify="flex-end" gap="sm">
            ...
            <Button onClick={handleGuardar} loading={guardando}>{modoEdicion ? 'Guardar cambios' : 'Crear unidad'}</Button>
          </Group>
        </Stack>
      </Modal>
      ...
      <Modal
        ...
        title={<Group gap="xs"><AlertTriangle size={18} color="var(--mantine-color-red-5)" /><Text fw={600} c="red">Eliminar unidad</Text></Group>}
        size="sm"
        centered
        overlayProps={{ blur: 3 }}
      >
        <Stack gap="md">
          ...
          <Group justify="flex-end" gap="sm">
            <Button variant="default" onClick={cerrarEliminar} disabled={eliminando}>Cancelar</Button>
            <Button color="red" onClick={handleEliminar} loading={eliminando} leftSection={<Trash2 size={14} />}>Eliminar</Button>
          </Group>
        </Stack>
      </Modal>
    </>
